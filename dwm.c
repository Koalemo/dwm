...
toggleattachbelow()
{
	attachbelowactive = !attachbelowactive;
}
...
createmon(void)
{
  ...
  /* ng: part of single tagset patch.
   * complicated logic to assign a tag to a new monitor.
   * not needed I dont think...
   * just open it with tag 1?
   */
  // Monitor *m, *tm;
  // int i;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? 1 << i : SchemeNorm]);
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? m->tagset[m->seltags] : SchemeNorm]);
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, scheme[selmon->tagset[selmon->seltags]][ColBorder].pixel);
		setfocus(c);
	} else {
...
focusstack(const Arg *arg)
{
	...
	if (arg->i > 0) {
		...
		if (!c)
			for (c = selmon->clients; c && !ISVISIBLE(c, selmon); c = c->next);
	} else {
...
tile(Monitor *m)
{
	...
	for (n = 0, c = nexttiled(m->clients, m); c; c = nexttiled(c->next, m), n++);
	if (n == 0)
		return;
	...
	if (n > m->nmaster)
		...
		mw = m->ww;
	for (i = my = ty = 0, c = nexttiled(m->clients, m); c; c = nexttiled(c->next, m), i++)
		if (i < m->nmaster) {
